// Bukkit Allocate - Allocate thread groups for each bukkit
...
@group(0) @binding(4) var<storage, read_write> g_bukkitParticleAllocator: array<atomic<u32>>;
...
fn csMain(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let particleStartIndex = atomicAdd(&g_bukkitParticleAllocator[0], bukkitCount);

    g_bukkitIndexStart[bukkitIndex] = particleStartIndex;

    for (var i: u32 = 0u; i < dispatchCount; i++) {
        var groupCount: u32 = ParticleDispatchSize;
        if (bukkitCountResidual != 0u && i == dispatchCount - 1u) {
