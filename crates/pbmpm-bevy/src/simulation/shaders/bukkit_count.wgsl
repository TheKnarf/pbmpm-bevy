// Bukkit Count - Count particles per spatial bucket
...
fn csMain(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    if (any(particleBukkit < vec2i(0i)) || u32(particleBukkit.x) >= g_simConstants.bukkitCountX || u32(particleBukkit.y) >= g_simConstants.bukkitCountY) {
        return;
    }

    let bukkitIndex = bukkitAddressToIndex(vec2u(particleBukkit), g_simConstants.bukkitCountX);
    atomicAdd(&g_bukkitCounts[bukkitIndex], 1u);
}
